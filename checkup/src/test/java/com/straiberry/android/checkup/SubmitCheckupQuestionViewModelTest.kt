...
class SubmitCheckupQuestionViewModelTest {
    ...
    private fun createViewModel() =
        CheckupQuestionSubmitTeethViewModel(
            addSeveralTeethToCheckupUseCase,
    ...
    fun `When checkup is not empty, then state should be loading`() = runTest {
        ...
        assertEquals(Loading, viewModel.submitStateAddTooth.value)
    }

    @Test
    fun `When teeth added to checkup successfully, then sate should be success`() = runTest {
        coEvery {
    ...
    @Test
    fun `When adding teeth to checkup is fail, then sate should be failure`() = runTest {
        coEvery {
            addSeveralTeethToCheckupUseCase.execute("test", ArrayList())
        } throws Exception()
